...
static void BleCarTask(void)
{
    ...
    while (1) {
        ...
        if (n < 0) {
            /* 读出错: 打印错误码 (限频, 每 10 次打一条) */
            err_cnt++;
            if (err_cnt % 10 == 1) {
                printf("[BleCar][诊断] 读蓝牙出错 ret=%d\r\n", (int)n);
            }
            osDelay(50);
            continue;
        }
        ...
        if (n == 0) {
            ...
            SendCar(cur_l, cur_r);
            /* 心跳: 每 10 次循环(约6秒)打一条, 证明任务没卡死 */
            if (loop_cnt % 10 == 1) {
                printf("[BleCar][心跳] 任务运行中 loop=%u err=%d\r\n",
                       (unsigned)loop_cnt, (int)err_cnt);
            }
            continue;
        }

        /* 收到字节: 原样打印十六进制, 便于诊断波特率/乱码问题 */
